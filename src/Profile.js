    ...
    return (
        <div className="profile-container">
            ...
            <div className="profile-field">
                <label>Имя:</label>
                {editMode ? <input name="first_name" value={userDetails.first_name} onChange={handleChange}/> :
                    <p>{userDetails.first_name}</p>}
            </div>
            <div className="profile-field">
                <label>Фамилия:</label>
                {editMode ? <input name="last_name" value={userDetails.last_name} onChange={handleChange}/> :
                    <p>{userDetails.last_name}</p>}
            </div>
            <div className="profile-field">
                <label>О себе:</label>
                {editMode ? <textarea name="bio" value={userDetails.bio} onChange={handleChange}/> :
                    <p>{userDetails.bio}</p>}
            </div>
            {isOwnProfile && (
                <button onClick={editMode ? handleSave : () => setEditMode(true)}>{editMode ? 'Save' : 'Edit'}</button>
            )}
            {!isOwnProfile && (
